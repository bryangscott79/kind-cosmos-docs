...
import { signals, Signal, industries, getSignalTypeLabel } from "@/data/mockData";
    ...
    <div className="flex items-center gap-3">
      {positive > 0 && (
        <div className="flex items-center gap-1">
          ...
          <span className="text-[10px] font-medium text-score-green">{positive} positive</span>
        </div>
      )}
      {negative > 0 && (
        <div className="flex items-center gap-1">
          <TrendingDown className="h-3 w-3 text-score-red" />
          <span className="text-[10px] font-medium text-score-red">{negative} negative</span>
        </div>
      )}
      {neutral > 0 && (
        <div className="flex items-center gap-1">
          <Minus className="h-3 w-3 text-muted-foreground" />
          <span className="text-[10px] font-medium text-muted-foreground">{neutral} neutral</span>
        </div>
      )}
    </div>
  );
...
function AffectedIndustries({ signalList }: { signalList: Signal[] }) {
  const industryIds = [...new Set(signalList.flatMap(s => s.industryTags))];
  const affected = industryIds
    ...
    <div className="flex flex-wrap gap-1.5 mt-3 pt-3 border-t border-border">
      ...
      {affected.map(ind => (
        <Link
          ...
          className="rounded-full bg-primary/5 border border-primary/10 px-2.5 py-0.5 text-[10px] font-medium text-primary hover:bg-primary/10 transition-colors"
        >
          {ind!.name}
        </Link>
      ))}
...
function CategorySection({ type, signalList, defaultOpen }: { type: Signal["signalType"]; signalList: Signal[]; defaultOpen: boolean }) {
    ...
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      ...
      {open && (
        <div className="px-5 pb-5 border-t border-border pt-4">
          ...
          <AffectedIndustries signalList={signalList} />
        </div>
      )}
    </div>
  );
...
export default function SignalFeed() {
  ...
  const types: { value: FilterType; label: string; count: number }[] = [
    ...
    ...categoryOrder.map(type => ({
      ...
      label: categoryConfig[type].label.split(" ")[0], // First word for compact filter
      count: signals.filter(s => s.signalType === type).length,
    })),
  ];

  const totalSources = filtered.reduce((sum, s) => sum + s.sources.length, 0);
    ...
    <DashboardLayout>
      ...
      {filter === "all" ? (
        <div className="mt-6 space-y-4">
          {categoryOrder
            .filter(type => groupedByCategory[type])
            .map((type, i) => (
              <CategorySection
                key={type}
                type={type}
                signalList={groupedByCategory[type]}
                defaultOpen={i < 2} // First two categories open by default
              />
            ))}
        </div>
      ) : (
        <div className="mt-6 space-y-3">
          ...
          <AffectedIndustries signalList={filtered} />
        </div>
      )}
    </DashboardLayout>
  );
