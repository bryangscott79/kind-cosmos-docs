...
function CollapsibleSection({ title, defaultOpen = false, badge, children }: { title: string; defaultOpen?: boolean; badge?: string; children: React.ReactNode }) {
      ...
      <DashboardLayout>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-6">
            ...
            <CollapsibleSection title="Recent Signals" defaultOpen={true} badge={`${relatedSignals.length} signals`}>
                      ...
                      {signal.sources?.length > 0 && (
                        <div className="ml-4 rounded-md border border-border bg-background p-3">
                          <div className="flex items-center gap-1.5 mb-2">
                            ...
                            <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Sources ({signal.sources.length})</span>
                          </div>
                          <div className="space-y-1.5">
                            {signal.sources.map((source, i) => (
                              <a key={i} href={source.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between group hover:bg-accent/50 rounded px-2 py-1 -mx-1 transition-colors">
